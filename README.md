*This project has been created as part of the 42 curriculum by vicperri.*
...
Data is stored in Docker-managed named volumes:
- **MariaDB**: `/var/lib/docker/volumes/mariadb/_data/`
- **WordPress**: `/var/lib/docker/volumes/wordpress/_data/`
...
# Backup volumes
docker compose exec mariadb mysqldump -u root -p${MYSQL_ROOT_PASSWORD} --all-databases > backup.sql
```

**Note**: You need `sudo` to access Docker volumes since they're owned by the Docker daemon.
...
**Volumes**: Named Docker volumes managed by Docker at `/var/lib/docker/volumes/`
...
**Implementation**: 
- **Docker Volumes** (recommended): Named volumes `mariadb` and `wordpress`
- **Storage location**: `/var/lib/docker/volumes/` (Docker managed)
- **Advantage**: Optimized performance, easy to backup, excellent portability
